...
class InputFeaturesFineTune(object):
    """A single set of features of data."""
...
class AtecCcksProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
...
def convert_examples_to_features_fine_tune(examples, label_list, max_seq_length,
                                 tokenizer, output_mode):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features.append(
                InputFeaturesFineTune(input_ids_a=input_ids_a,
