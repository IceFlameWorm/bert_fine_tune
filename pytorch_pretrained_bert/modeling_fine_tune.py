...
class BertForPairWiseClassification(BertPreTrainedModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
...
class BertForPairWiseClassification2(BertPreTrainedModel):
    """BERT model for classification.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    ...
    """
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(BertForPairWiseClassification2, self).__init__(config)
        self.output_attentions = output_attentions
        self.num_labels = num_labels
        self.bert = BertModel(config, output_attentions=output_attentions,
                                      keep_multihead_output=keep_multihead_output)
    ...
    def contractive_sim(self, out1, out2):
        eps = 1e-5
        dis = torch.norm(out1 - out2, p=2, dim = 1)
        norm1 = torch.norm(out1, p=2, dim=1)
        norm2 = torch.norm(out2, p=2, dim=1)
        sim = 1 - dis / (norm1 + norm2 + eps)
        sim = torch.clamp(sim, min = eps, max = 1-eps)
        return sim
    ...
    def forward(self, input_ids_1, input_ids_2,
        ...
        # logits = self.classifier(pooled_output)
        pos_prob = self.contractive_sim(pooled_output_1, pooled_output_2)
        neg_prob = 1 - pos_prob
        probs = torch.stack([neg_prob, pos_prob], dim = 1) # Shape: Batch_size X 2
        log_probs = torch.log(probs)
        ...
        if labels is not None:
            ...
            return all_attentions_1, all_attentions_2, cos_sim, pos_prob, pooled_output_1, pooled_output_2
        return pos_prob, pooled_output_1, pooled_output_2
